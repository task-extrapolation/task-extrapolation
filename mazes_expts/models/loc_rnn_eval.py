...
class RNNSegm(nn.Module):
    def __init__(self, rnn_name, depth, width):
        super().__init__()
        self.rnn_name = rnn_name
        self.inplanes = depth
        self.widths = 2  # hard coded, following ResidualNetworkSegment style of model
        self.inplanes = self.inplanes * self.widths
        self.timestep=width
        ...
        if self.rnn_name.startswith("locrnn"):
            ...
            self.rnn = ConvGRU(self.inplanes, self.inplanes, 5, self.timestep)
        else:
    ...
    def forward_viz(self, x):  
        ...
        l_x = self.rnn(x)
        for t in range(len(l_x)):
            thought = F.relu(self.bn2(self.conv2(l_x[t])))
            thought = F.relu(self.bn3(self.conv3(thought)))
...
# widths = 2 is fixed in the class defined above, as the experiment used width = 2 across all models, 
# which represents a numeric factor that double the inplanes (depth). 

# 'width' in the functions below represents the timestep, which is used in eval_mazes.py
def locrnn(num_outputs, depth, width, dataset):
    return RNNSegm("locrnn", depth, width)

def hgru(num_outputs, depth, width, dataset):
    return RNNSegm("hgru", depth, width)

def gru(num_outputs, depth, width, dataset):
    return RNNSegm("gru", depth, width)